...
class ApplicationTest(TestCase):
    ...
    def test_application_name_is_unique(self):
        ...
        create_application_url = reverse('new_application_form',
        ...
        self.assertEquals(
            Application.objects.filter(project=project).count(), 1L,
            'The view must not accept to create two applicationis with '
            'the same name in the same project')

        self.assertFormError(
            response, 'new_application_form', 'name',
    ...
    def test_application_view(self):
        ...
        view_application_url = reverse(
            'application_view', kwargs={'application_id': application.id})
    ...
    def test_application_name_casting(self):
        ...
        for name, casted in name_collection.items():
            ...
            self.assertTrue(application.name == casted,
                            'Error application name casting ("%s!=%s")' % (
                            casted, application.name))

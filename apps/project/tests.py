...
from apps.form.models import ModelForm
from apps.field.models import ModelField, CharField
...
class ProjectTest(TestCase):
    ...
    def test_project_detail_view(self):
        ...
        view_project_detail_url = reverse('project_view',
    ...
    def test_project_name_is_unique(self):
        ...
        for post_opts in post_opts_list:
            ...
            self.assertEquals(
                Project.objects.filter(name=project.name).count(), 1L,
                'The view must not accept to create two projects with '
                'the same name and same owner')
    ...
    def test_project_delete(self):
        project = Project.objects.get(name='test project')
        delete_project_url = reverse('project_del',
                                     kwargs={'project_id': project.id})
    ...
    def test_project_creation(self):
        ...
        self.assertTrue(
            projects[0] in response.context['projects'].object_list)
    ...
    def test_project_del(self):
        ...
        self.assertTrue(
            projects.count() == 1L,
            'user "username" must have only one project in '
            'fixture project.json')

        project = projects[0]
        application_ids = project.applications.values_list('id', flat=True)
        model_ids = Model.objects.filter(
            application__project=project).values_list('id', flat=True)
        form_ids = ModelForm.objects.filter(
            model__application__project=project).values_list('id', flat=True)

        # lets insert a field into the project
        model_field = self.insert_field(
            Model.objects.filter(application__project=project)[0],
            max_length=255)
        ...
        self.assertEqual(
            ...
            'The call the del view (%s) does not behavior has '
            'expected.' % delete_project_url)
        ...
        self.assertTrue(
            ModelField.objects.filter(pk=model_field.id).count() == 0,
